...
def attachment_result(answers):
    ...
    anxious_score = anxious / 20
    avoidant_score = avoidant / 10
    secure_score = secure / 20

    # Тревожно-избегающий стиль
    if (
        anxious_score >= 0.7
        and avoidant_score >= 0.7
    ):

        result_type = "Тревожно-избегающий"

        description = (
            "Ты можешь одновременно сильно хотеть "
            "близости и бояться её. Поэтому иногда "
            "может появляться желание приблизиться "
            "к человеку, а потом резко отдалиться."
        )

        advice = (
            "Старайся не принимать решения на пике "
            "эмоций. Если тебе становится тревожно "
            "или хочется резко отдалиться, сначала "
            "дай себе время успокоиться, а затем "
            "прямо скажи человеку, что ты чувствуешь "
            "и чего хочешь."
        )

    # Тревожный стиль
    elif anxious_score >= 0.65:

        result_type = "Тревожный"

        description = (
            "Отношения могут быть для тебя очень "
            "эмоционально значимыми. Если человек "
            "становится холоднее, отвечает реже "
            "или ведёт себя иначе, ты можешь быстро "
            "начать переживать и искать объяснение."
        )

        advice = (
            "Не оценивай отношения по каждому "
            "сообщению. Старайся смотреть на общую "
            "динамику поведения человека и не искать "
            "постоянного подтверждения его чувств."
        )

    # Избегающий стиль
    elif avoidant_score >= 0.7:

        result_type = "Избегающий"

        description = (
            "Тебе может быть важно сохранять личное "
            "пространство и независимость. Когда "
            "отношения становятся слишком интенсивными, "
            "может появляться желание дистанцироваться "
            "или побыть одному."
        )

        advice = (
            "Важно не исчезать из отношений только "
            "из-за временного дискомфорта. Попробуй "
            "объяснять человеку, когда тебе нужно "
            "личное пространство, вместо того чтобы "
            "просто закрываться или отдаляться."
        )
    ...
    # Надёжный стиль
        ...
        description = (
            "Ты в целом способен сохранять близость, "
            "доверие и личные границы одновременно. "
            "Изменения в поведении партнёра не обязательно "
            "сразу воспринимаешь как угрозу отношениям."
        )

        advice = (
            "Сохраняй открытость и продолжай прямо "
            "говорить о своих чувствах и потребностях. "
            "При этом не забывай уважать личное пространство "
            "как своё, так и другого человека."
        )
...
async def handle_attachment_test(
    ...
    result = (
        "🧪 ТЕСТ НА СТИЛЬ ПРИВЯЗАННОСТИ\n\n"
        f"❤️ ТВОЙ РЕЗУЛЬТАТ: {result_type}\n\n"
        f"{description}\n\n"
        f"🎯 ЧТО ВАЖНО:\n\n"
        f"{advice}\n\n"
        "Важно: этот тест не является "
        "медицинской или психологической диагностикой. "
        "Он показывает только общую тенденцию "
        "по твоим ответам."
    )
